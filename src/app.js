...
(function () {
  ...
  var MODE_PROFILES = {
    ...
    en: {
      ...
      maxCharsTitle: '英文只在 . ! ? ; : 标点后切分，合并后不超过该字符数',
      maxChars: 80,
      speedLabel: '字符耗时（秒/字符）',
      speedTitle: '每个英文字符（含空格与标点）的朗读耗时，参考默认 0.045 秒',
      speed: 0.045,
      speedStep: 0.001,
      speedMin: 0.001,
      gapMs: 250
  ...
  function applyModeProfile(mode) {
    ...
    setResult('已切换到' + (mode === 'en' ? '英文字幕' : '中文字幕') + '模式。', true);
  }
  ...
  function readSettings() {
    ...
    var settings = {
      mode: mode,
      maxChars: Math.max(1, Math.floor(Number(maxCharsInput.value) || profile.maxChars)),
      gapMs: Number(gapMsInput.value) >= 0 ? Number(gapMsInput.value) : profile.gapMs
    };
    var speed = Number(speedInput.value) > 0 ? Number(speedInput.value) : profile.speed;
    if (mode === 'en') {
  ...
  choosePathBtn.addEventListener('click', async function () {
    if (!window.showSaveFilePicker) {
      setPath('当前浏览器不支持“选择保存位置”，点击“生成”将直接下载 SRT 文件。');
      return;
    }
    try {
