...
const out = srt.generateSrt('第一句。第二句？第三句！', { maxChars: 5, speed: 5, gapMs: 200 });
...
// 5. 时间轴累计与间隔
const out2 = srt.generateSrt('一二三四五，一二三四五。', { maxChars: 7, speed: 5, gapMs: 200 });
...
const s6 = srt.splitIntoSentences('先讲清楚一个观念：我们为什么要手动去摆这些姿势？', 20);
assert.deepStrictEqual(s6, ['先讲清楚一个观念', '我们为什么要手动去摆这些姿势']);

const s7 = srt.splitIntoSentences('是为程序化动画设计的——动画是系统根据数据自动生成的，', 20);
assert.deepStrictEqual(s7, ['是为程序化动画设计的', '动画是系统根据数据自动生成的']);
...
const s16 = srt.splitIntoSentences('这一集我们来测试一下Blender版的大提琴手林尼', 20);
assert.deepStrictEqual(s16, ['这一集我们来测试一下Blender版的大提琴手林尼']);

// 行内后面还有分隔符时，仍按字数上限硬切，下一轮再按分隔符收尾
const s17 = srt.splitIntoSentences('一二三四五六七八九十一二三四五六七八九十一二，三四五', 20);
assert.deepStrictEqual(s17, ['一二三四五六七八九十一二三四五六七八九十', '一二', '三四五']);
